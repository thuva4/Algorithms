...
function simpleTernarySearch(itemList) {
  ...
  while (left <= right) {
    ...
    if (itemList[leftThird] < itemList[rightThird]) left = leftThird;
...
function ternarySearch(func, left, right, absolutePrecision) {
  ...
  while (true) {
    ...
    if (func(leftThird) < func(rightThird)) left = leftThird;
    else right = rightThird;
  }
}
...
function ternarySearchRecursive(func, left, right, absolutePrecision) {
  // left and right are the current bounds. the maximum is between them
  if (Math.abs(right - left) < absolutePrecision) return Math.floor((left + right) / 2);
...
// This list must be sorted. If it is not given as sorted, sort it first
...
module.exports = { ternarySearch, ternarySearchRecursive };
