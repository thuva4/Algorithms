...
def compile_binary(algo_dir: Path, sources: list[Path], wrapper_source: str) -> tuple[Path | None, str | None]:
    ...
    try:
        ...
        staging_binary = build_dir / f"runner.{os.getpid()}.{threading.get_ident()}.tmp"
        shutil.copy2(tmp_dir / "runner", staging_binary)
        os.replace(staging_binary, binary)
        return binary, None
    finally:
...
def run_algorithm(algo_dir: Path) -> AlgorithmResult:
    ...
    exec_dir = Path(tempfile.mkdtemp(prefix="go-runner-exec-"))
    run_binary = exec_dir / "runner"
    try:
        source_mode = binary.stat().st_mode
        with binary.open("rb") as src, run_binary.open("wb") as dst:
            shutil.copyfileobj(src, dst)
            dst.flush()
            os.fsync(dst.fileno())
        run_binary.chmod(source_mode | 0o111)
        ...
        for attempt in range(5):
            ...
            if last_os_error is not None:
                raise last_os_error
            raise RuntimeError("Failed to launch Go runner")
    except subprocess.TimeoutExpired:
        result.failed += len(test_cases)
        ...
        result.failed += len(test_cases)
        result.errors.append(f"{algo_name}: Runtime error: {exc}")
        return result
    finally:
